...
const TableThree : React.FC<any> = ({packageData,setSuccess,setError,setPackageData}) => {
  const deleteHandler = async(id: string) => {
    console.log(id);
    try {
      const res = await axios.delete(`http://localhost:8000/api/v1/program/deleteprogram`,{
        data: {
          program_id: id,
        }
      });
            ...
            {packageData?.map((packageItem:any, key:number) => (
              <tr key={key}>
                <td className="border-b border-[#eee] py-5 px-4 pl-9 dark:border-strokedark xl:pl-11">
                  <img src={packageItem.image} alt={packageItem.heading} className="w-20 h-20 object-cover" />
                </td>
                ...
                <td className="border-b border-[#eee] py-5 px-4 dark:border-strokedark">
                  <div className="flex flex-wrap gap-2" >
                      {packageItem.keywords?.map((keyword:any, key:number) => (
                        <p
                        key={key}
                        className={`inline-flex rounded-full bg-opacity-10 py-1 px-3 text-sm font-medium bg-success text-success`}
                      >
